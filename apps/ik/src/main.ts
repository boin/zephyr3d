...
btnTarget.addEventListener('click', () => setActiveHandle('target'));
btnPole.addEventListener('click', () => setActiveHandle('pole'));
...
function setActiveHandle(handle: IKHandle) {
  activeHandle = handle;
  btnTarget.classList.toggle('active', handle === 'target');
  btnPole.classList.toggle('active', handle === 'pole');
}
