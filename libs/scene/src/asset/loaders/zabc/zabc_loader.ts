...
export async function attachZABCAnimationsToSceneNode(
  ...
  for (const animation of parsed.content.animations ?? []) {
    ...
    for (const track of animation.tracks ?? []) {
      ...
      if (!targetMesh) {
        const subMeshLabel =
          typeof track.subMeshIndex === 'number' && track.subMeshIndex >= 0
            ? `${track.subMeshIndex}`
            : 'auto';
...
function remapGeometryCacheFrames(
...
function remapPCAGeometryCacheData(
  subMesh: AssetSubMeshData,
  track: PCAGeometryCacheTrackData
...
async function remapPCAGeometryCacheDataForMesh(
  mesh: Mesh,
  track: PCAGeometryCacheTrackData
): Promise<PCAGeometryCacheTrackData> {
  const rawPositions = await getMeshRawPositions(mesh);
  const remapReference = track.positionReference ?? reconstructPCAGeometryCacheReference(track);
...
function extractPositionArray(bytes: Uint8Array<ArrayBuffer>, info: VertexBufferInfo) {
  const componentSize =
    ((info.type as unknown as { getSize(): number }).getSize?.() ?? 0) / Math.max(1, info.type.cols);
  if (componentSize !== 4) {
    return null;
  }
